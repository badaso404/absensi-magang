...
<div class="page-content">
            ...
            <div class="card summary-card shadow-sm">
                <div class="card-body d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="h5 font-weight-bold">{{ $absensi->count() }}</div>
                ...
                <div class="card-body d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="h5 font-weight-bold">
                            {{ $absensi->where('checked_in_status', 'on_time')->count() }}
                        </div>
            ...
            <div class="card summary-card shadow-sm">
                <div class="card-body d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <div class="h5 font-weight-bold">
                            {{ $absensi->where('status', '!=', 'present')->count() }}
                        </div>
    ...
    <div class="card card-fluid shadow-sm">
         <div class="card-header bg-gradient-primary text-white d-flex justify-content-between align-items-center">
            ...
            <small class="text-light-50">{{ $absensi->count() }} entri</small>
        </div>
                        ...
                        <tr class="cursor-pointer" data-toggle="tooltip" title="Klik untuk detail"
                            ...
                            <td>
                                <div class="d-flex flex-column">
                                    ...
                                    <small class="text-muted">{{ \Carbon\Carbon::parse($a->created_at)->isoFormat('D MMMM Y') }}</small>
                                </div>
                            </td>

                            <td>
                                <div>
                                    <div class="font-weight-bold text-{{ optional($a->checked_in_status)->color() ?? 'muted' }}">
                                        {{ optional($a->checked_in_status)->text() ?? '-' }}
                                    </div>
                                    <small class="text-muted">{{ $a->checked_in_at ? \Carbon\Carbon::parse($a->checked_in_at)->isoFormat('HH:mm:ss') : '-' }}</small>
                                </div>
                            </td>

                            <td>
                                <div>
                                    <div class="font-weight-bold text-{{ optional($a->checked_out_status)->color() ?? 'muted' }}">
                                        {{ optional($a->checked_out_status)->text() ?? '-' }}
                                    </div>
                                    <small class="text-muted">{{ $a->checked_out_at ? \Carbon\Carbon::parse($a->checked_out_at)->isoFormat('HH:mm:ss') : '-' }}</small>
                                </div>
                            </td>

                            <td>
                                <span class="badge badge-pill badge-{{ optional($a->status)->color() ?? 'secondary' }}">
                                    <i class="fas {{ optional($a->status)->icon() ?? 'fa-info-circle' }} mr-1"></i>
                                    {{ optional($a->status)->text() ?? '-' }}
                                </span>
                                @if($a->note)
                                    <div><small class="text-muted d-block d-md-none">{{ \Illuminate\Support\Str::limit($a->note,60) }}</small></div>
                                @endif
                            </td>

                            <td class="d-none d-md-table-cell">
                                @if($a->wfhwfo)
                                    <span class="badge badge-{{ $a->wfhwfo == 'WFH' ? 'success' : 'primary' }}">
                                        {{ $a->wfhwfo }}
                                    </span>
                                @else
                            ...
                            <td>
                               ...
                               @if($a->lokasi_user || ($a->latitude && $a->longitude))
                                        <div class="d-flex flex-column">
                                            <div class="mb-2">
                                                <i class="fas fa-map-marker-alt text-danger mr-1"></i>
                                                <small class="text-muted">
                                                    {{ $a->lokasi_user ?? 'Lokasi tidak tersedia' }}
                                                </small>
                                            </div>
                                            @if($a->latitude && $a->longitude)
                                                <div class="d-flex align-items-center">
                                                    <button 
                                                        ...
                                                        data-lokasi="{{ $a->lokasi_user ?? 'Lokasi Absen' }}"
                                                        data-toggle="tooltip"
                                                    ...
                                                    <small class="text-muted ml-2">
                                                        ...
                                                        {{ number_format($a->latitude, 6) }},<br> {{ number_format($a->longitude, 6) }}
                                                    </small>
                                                </div>
                                            @endif
                                        </div>
